...
from data.inpaint_dataset_my import InpaintDatasetMy
...
def validate(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=cuda0, batch_n="whole"):
    ...
    info = {}
    for i, (imgs, masks, drawings) in enumerate(dataloader):

        data_time.update(time.time() - end)

        imgs, masks, drawings = imgs.to(device), masks.to(device), drawings.to(device)
        imgs = (imgs / 127.5 - 1)
        masks = masks / 255.0
        drawings = drawings / 255.0

        coarse_imgs, recon_imgs, attention = netG.forward(imgs, masks, drawings)

        complete_imgs = recon_imgs * masks + imgs * (1 - masks)

        pos_imgs = torch.cat([imgs, masks, drawings], dim=1)
...
def train(netG, netD, GANLoss, ReconLoss, DLoss, optG, optD, dataloader, epoch, device=cuda0, val_datas=None):
    ...
    for i, (imgs, masks, drawings) in enumerate(dataloader):
        ...
        coarse_imgs, recon_imgs, attention = netG(imgs, masks, drawings)
